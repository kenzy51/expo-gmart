...
const Browse = ({navigation, route}) => {
  ...
  useEffect(() => {
    ...
    Api.getProducts(args, async (err, result) => {
      if (
        ...
        setSortData(result?.data?.products);
      else setSortData([]);
  ...
  return (
    <View style={styles.container}>
      {screenName === 'Home' ? (
        <TopnavBar
          title={categoriesName ? categoriesName : 'Browse'}
          from={'back'}
        />
      ) : (
        <TopnavBar
          title={categoriesName ? categoriesName : 'Browse'}
          from={categoriesId ? null : 'Home'}
        />
      )}
      ...
      {loading ? (
        ...
        <>
          <FlatList
            ...
            data={sortData}
            renderItem={({item}) => (
              <ProductCom from={'horizontolCart'} item={item} />
            )}
            ListEmptyComponent={() => (
